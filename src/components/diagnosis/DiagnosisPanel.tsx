...
export function DiagnosisPanel({alarm}:{alarm?:Alarm}){
 const d=diagnose(alarm);
 return <Card title="AI 模擬診斷" className="diagnosis">
  <div className="ai-title"><BrainCircuit/><div><small>可能根本原因</small><strong>{d.rootCause}</strong></div></div>
  <div className="confidence"><span style={{width:`${d.confidence}%`}}/><b>{d.confidence}% 信心分數</b></div>
  <h3>判斷依據</h3><ol>{d.evidence.map(x=><li key={x}>{x}</li>)}</ol>
  <div className="callout"><ShieldAlert size={18}/><span><b>影響：</b>{d.impact}<br/><b>建議：</b>{d.steps.join(' → ')}</span></div>
  {d.maintenanceLikely&&<div className="maintenance-note">此告警可能由既定維護作業造成，請先確認維護進度。</div>}
