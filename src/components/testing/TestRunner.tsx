...
export function TestRunner({incidentId}:{incidentId?:string}){
 ...
 useEffect(()=>{if(!running)return; const idx=items.findIndex(x=>x.status==='等待'); if(idx<0){setRunning(false);notify('模擬查測完成：疑似光纖或 SFP 異常');return}
  setItems(v=>v.map((x,i)=>i===idx?{...x,status:'執行中'}:x));
  const t=setTimeout(()=>setItems(v=>v.map((x,i)=>i===idx?{...x,status:idx<5||idx===9?'失敗':'成功'}:x)),380); return()=>clearTimeout(t);
 },[items,running,notify]);
 const start=()=>{setItems(seed);setRunning(true); if(incidentId)addTimeline(incidentId,'開始一鍵模擬查測')};
