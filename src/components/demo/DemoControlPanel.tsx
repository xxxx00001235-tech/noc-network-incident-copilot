...
export function DemoControlPanel(){
 ...
 return <div className="demo-control">
  <button className={`demo-trigger ${active?'active':''}`} onClick={()=>setOpen(value=>!value)} aria-expanded={open}><Play/>{active?'情境執行中':'開發情境'}<ChevronDown/></button>
  {open&&<div className="demo-panel" role="dialog" aria-label="開發情境控制"><header><div><small>DEVELOPER UTILITY</small><b>障礙情境控制</b></div><button className="icon" onClick={()=>setOpen(false)} aria-label="關閉"><X/></button></header>
