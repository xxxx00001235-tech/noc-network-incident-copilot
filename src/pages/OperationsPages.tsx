...
import { fetchAlarms, fetchLatestAlarm } from '../api/alarms';
import { generateTeamsReport, type ReportResponse } from '../api/report';
import { ApiError } from '../lib/apiClient';
import { alarmListRefreshEvent } from '../api/alarmSocket';

export function AlarmPage(){
 const alarms=useNocStore(s=>s.alarms),realtimeState=useNocStore(s=>s.realtimeState),selectedId=useNocStore(s=>s.selectedAlarmId),select=useNocStore(s=>s.selectAlarm),regionFilter=useNocStore(s=>s.regionFilter),setRegion=useNocStore(s=>s.setRegionFilter); const [q,setQ]=useState(''),[sev,setSev]=useState(''),[status,setStatus]=useState('');
 const[apiAlarms,setApiAlarms]=useState<Alarm[]|null>(null),[alarmApiState,setAlarmApiState]=useState<'loading'|'success'|'error'>('loading'),[alarmApiError,setAlarmApiError]=useState(''),[alarmRefresh,setAlarmRefresh]=useState(0);
 useEffect(()=>{const refresh=()=>setAlarmRefresh(value=>value+1);window.addEventListener(alarmListRefreshEvent,refresh);return()=>window.removeEventListener(alarmListRefreshEvent,refresh)},[]);
 useEffect(()=>{
  let active=true;
  setAlarmApiState('loading');setAlarmApiError('');
  void fetchAlarms().then(response=>{if(!active)return;setApiAlarms(response);setAlarmApiState('success')}).catch(error=>{if(!active)return;setAlarmApiState('error');setAlarmApiError(error instanceof ApiError?error.message:'無法載入告警 API')});
  return()=>{active=false};
 },[alarmRefresh]);
 const displayAlarms=apiAlarms??alarms;
 ...
 <div className={`api-state ${alarmApiState}`}>{alarmApiState==='loading'?<><RefreshCw className="spin"/><span><b>正在載入 PostgreSQL 告警…</b><small>透過 FastAPI GET /alarms 讀取最新資料</small></span></>:alarmApiState==='success'?<><Check/><span><b>PostgreSQL 告警已載入</b><small>FastAPI GET /alarms · {displayAlarms.length} 筆資料</small></span></>:<><WifiOff/><span><b>告警 API 無法連線</b><small>{alarmApiError}；目前保留既有告警介面與資料。</small></span><button className="btn small" onClick={()=>setAlarmRefresh(value=>value+1)}><RefreshCw/>重新載入</button></>}</div>
