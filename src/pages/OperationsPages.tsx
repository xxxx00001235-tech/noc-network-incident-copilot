...
export function AlarmPage(){
 ...
 useEffect(()=>{
  ...
  void fetchAlarms().then(response=>{if(!active)return;setApiAlarms(response);setAlarmApiState('success')}).catch(error=>{if(!active)return;setApiAlarms(null);setAlarmApiState('error');setAlarmApiError(error instanceof ApiError?error.message:'無法載入告警 API')});
  return()=>{active=false};
 },[alarmRefresh]);
