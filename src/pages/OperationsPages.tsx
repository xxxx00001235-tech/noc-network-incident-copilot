...
import { isActiveAlarm, isFormalAlarm } from '../store/selectors';

export function AlarmPage(){
 const alarms=useNocStore(s=>s.alarms),realtimeState=useNocStore(s=>s.realtimeState),selectedId=useNocStore(s=>s.selectedAlarmId),select=useNocStore(s=>s.selectAlarm),regionFilter=useNocStore(s=>s.regionFilter),setRegion=useNocStore(s=>s.setRegionFilter),alarmApiState=useNocStore(s=>s.alarmDataState),alarmApiError=useNocStore(s=>s.alarmDataError),refreshAlarms=useNocStore(s=>s.refreshAlarms); const [q,setQ]=useState(''),[sev,setSev]=useState(''),[status,setStatus]=useState('');
 const displayAlarms=alarms.filter(isFormalAlarm);
 ...
 return <div className="page"><div className="page-title"><div><span className="eyebrow">ALARM COMMAND CENTER</span><h1>告警中心</h1><p>所有頁面共用 PostgreSQL／FastAPI 告警資料；離線時保留最後一次成功同步結果。</p></div><Badge tone="critical">{displayAlarms.filter(isActiveAlarm).length} 筆作用中</Badge></div>
 <div className={`api-state ${alarmApiState==='idle'?'loading':alarmApiState}`}>{alarmApiState==='loading'||alarmApiState==='idle'?<><RefreshCw className="spin"/><span><b>正在載入 PostgreSQL 告警…</b><small>透過 FastAPI GET /alarms 讀取最新資料</small></span></>:alarmApiState==='success'?<><Check/><span><b>PostgreSQL 告警已載入</b><small>FastAPI GET /alarms · {displayAlarms.length} 筆資料</small></span></>:<><WifiOff/><span><b>資料服務暫時離線</b><small>{alarmApiError}；目前顯示最後一次成功同步結果。</small></span><button className="btn small" onClick={()=>void refreshAlarms()}><RefreshCw/>重新連線</button></>}</div>
 <div className={`api-state ${realtimeState==='connected'?'success':realtimeState==='connecting'?'loading':'error'}`}>{realtimeState==='connected'?<><Check/><span><b>FastAPI WebSocket 即時連線</b><small>新告警會自動更新所有營運畫面</small></span></>:realtimeState==='connecting'?<><RefreshCw className="spin"/><span><b>正在連接 FastAPI WebSocket…</b><small>連線期間仍顯示現有告警資料</small></span></>:<><WifiOff/><span><b>FastAPI WebSocket 已斷線</b><small>系統會自動重新連線，目前保留現有告警資料</small></span></>}</div>
 <div className="toolbar"><label className="search"><Search/><input placeholder="搜尋設備、IP、告警內容…" value={q} onChange={e=>setQ(e.target.value)}/></label><Filter/><select value={sev} onChange={e=>setSev(e.target.value)}><option value="">所有嚴重度</option>{['Critical','Major','Minor','Warning','Normal'].map(x=><option key={x}>{x}</option>)}</select><select value={regionFilter} onChange={e=>setRegion(e.target.value)}><option value="">所有區域</option>{[...new Set(displayAlarms.map(a=>a.region))].map(x=><option key={x}>{x}</option>)}</select><select value={status} onChange={e=>setStatus(e.target.value)}><option value="">所有狀態</option>{[...new Set(displayAlarms.map(a=>a.status))].map(x=><option key={x}>{x}</option>)}</select></div>
...
export function IncidentsPage(){
 const allIncidents=useNocStore(s=>s.incidents),allAlarms=useNocStore(s=>s.alarms),selectedAlarmId=useNocStore(s=>s.selectedAlarmId),selectAlarm=useNocStore(s=>s.selectAlarm),update=useNocStore(s=>s.updateIncident),add=useNocStore(s=>s.addTimeline),notify=useNocStore(s=>s.notify);const alarms=allAlarms.filter(isFormalAlarm),incidents=allIncidents.filter(incident=>alarms.some(alarm=>alarm.incidentId===incident.id));const[note,setNote]=useState('');const alarm=alarms.find(a=>a.id===selectedAlarmId)??alarms[0];const i=incidents.find(x=>x.id===alarm?.incidentId)??incidents[0];
 ...
 return <div className="page"><div className="page-title"><div><span className="eyebrow">INCIDENT LIFECYCLE</span><h1>事件中心</h1><p>事件與 AI 診斷共用目前選取的告警。</p></div><Badge tone="critical">{alarms.filter(isActiveAlarm).length} 筆作用中</Badge></div><div className="incident-tabs">{incidents.map(x=><button key={x.id} className={x.id===i.id?'active':''} onClick={()=>{const target=alarms.find(item=>item.incidentId===x.id);if(target)selectAlarm(target.id)}}><Badge tone={severityTone(x.severity)}>{x.severity}</Badge><b>{x.title}</b><small>{x.id}</small></button>)}</div>
...
export function DiagnosisPage(){
 ...
 useEffect(()=>{let active=true;if(!a){setTimeline([]);setState('idle');return()=>{active=false}}setState('loading');Promise.all([fetchAnalysis(a.deviceId),fetchAiTimeline(a.deviceId)]).then(([nextAnalysis,nextTimeline])=>{if(active){setAnalysis(nextAnalysis);setTimeline(nextTimeline.events.length?nextTimeline.events:fallbackAiTimeline(a));setState('success')}}).catch(()=>{if(active){setAnalysis(null);setTimeline(fallbackAiTimeline(a));setState('error')}});return()=>{active=false}},[a?.deviceId,a?.id,a?.updated]);
 return <div className="page"><div className="page-title"><div><span className="eyebrow">NOC AI COPILOT</span><h1>AI 診斷工作台</h1><p>AI 診斷、時間軸與事件中心共用目前選取的告警。</p></div></div><div className="toolbar"><select value={a?.id??''} onChange={e=>select(e.target.value)}>{alarms.map(x=><option value={x.id} key={x.id}>{x.id} · {x.deviceName}</option>)}</select></div>{state==='error'&&<div className="api-state error"><WifiOff/><span><b>AI 分析服務暫時無法連線</b><small>目前顯示既有告警與事件所建立的分析結果。</small></span></div>}<div className="split equal"><DiagnosisPanel alarm={a} analysis={analysis} analysisState={state}/><Card title="AI Timeline"><div className="timeline ai-timeline">{timeline.map(event=><div key={event.stage}><time>{event.stage}</time><i/><span><b>{event.detail}</b><small>{event.actor} · {new Date(event.time).toLocaleString('zh-TW')}</small></span></div>)}</div></Card></div><div className="split equal"><TestRunner incidentId={a?.incidentId}/></div></div>
}
